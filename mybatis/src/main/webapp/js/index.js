...
$(window).load(function () {
    ...
    function maxHeight() {
        var arr = [];
        arr[0] = $(document).innerHeight() - (70 + 21);
        arr[1] = $('.tab-content>.active').innerHeight() + 32;
        arr[2] = $('.menu-body').innerHeight() + 29;
...
$(function () {
    ...
    MenuTab.prototype = {
        ...
        addTab: function (ev) {
            ...
            //获取页面ID
            newTabData.pageID=$target.parent().prop("id");
        ...
        addTabContent: function (newTabData) {
            var $content = $('<div class="tab-pane active" id="' + newTabData.pageName + '"><iframe frameborder="0" src="' + newTabData.fileName + '/' + newTabData.pageName + '.html?id='+newTabData.pageID + '" name="' + newTabData.pageName + '"></iframe></div>');
    ...
    Menu.prototype = {
        init: function () {
            ...
            this.$menu.on('click', '>li>a', function () {
                that.toggleMenuList($(this));
                return false;
            });
            this.$menu.on('click', '>li>ul>li', function () {
                var $nav, addTabEvent = $.Event('addTab.tab', {
                    relatedTarget: $(this).find('a')[0]
                });
                $nav = $('.nav-wraper .nav-tabs');
                that.toggleMenuItem($(this));
                $nav.trigger(addTabEvent);
                //防止地址栏显示信息
                return false;
            });
        },
        toggleMenuList: function ($el) {
